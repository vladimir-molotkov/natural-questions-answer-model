...
def create_gpt_dataloader(dataset, tokenizer, batch_size=8):
    def tokenize_fn(examples):
        texts = [f"Question: {q} Answer: {a}" for q, a in zip(examples["question"], examples["answer"])]
        tokenized = tokenizer(
            texts,
            max_length=256,
            truncation=True,
...
def train_gpt(model_name="gpt2", train_sample_size=5000, val_sample_size=1000, epochs=3):
    model = GPT2QAModel(model_name)
